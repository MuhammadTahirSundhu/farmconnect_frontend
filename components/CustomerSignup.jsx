...
import { insertConsumer } from "@/services/consumerServiceApi";
...
const CustomerSignup = ({ closeForm }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const consumerData = { name, email, password, location };
    ...
    try {
      ...
      // API call
      const response = await insertConsumer(consumerData);
    ...
    <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center z-50">
      ...
      <div className="relative flex flex-col items-center w-[400px] p-6 bg-transparent backdrop-blur-lg rounded-xl shadow-xl border border-gray-200/50">
        ...
        <form onSubmit={handleSubmit} className="w-full space-y-4">
          ...
          <button
            type="submit"
            className="w-full p-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500"
          >
            Submit
          </button>
        </form>
